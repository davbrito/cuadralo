...
- UI stack: Tailwind v4, shadcn-style UI components in `app/components/ui`, Hugeicons.
  - Note: The `app/components/ui` folder is reserved for shadcn-managed components. Do not add or modify components there manually; use `pnpm ui:add` to add components so the shadcn tooling keeps the folder consistent.
